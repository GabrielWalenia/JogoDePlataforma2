#include "pistol.h"

pistol* pistol_create(){

    pistol *new_pistol = malloc(sizeof(pistol));
    if(!new_pistol) return NULL;

    new_pistol->timer = 0;
    new_pistol->shots = NULL;
    
    return new_pistol;
}

bullet *pistol_shot(int x, int y, int dir, pistol *gun){
    bullet *new_bullet = bullet_create( x, y, dir, gun->shots);
    if(!new_bullet) return NULL;
    return new_bullet;
}

void pistol_destroy(pistol *elemento){
    bullet *sentinela;
    for(bullet *index = (bullet *) elemento->shots; index!=NULL; index = (bullet *) sentinela){
        sentinela = (bullet *) index->next;
        bullet_destroy(index);
    }
    free(elemento);
}
